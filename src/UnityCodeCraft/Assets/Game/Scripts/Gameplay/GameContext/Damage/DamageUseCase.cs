...
namespace Game.Gameplay
{
    public static partial class DamageUseCase
    {
        ...
        public static void DealDamageOverlap(this IGameEntity entity, GameContext gameContext, Collider[] results,
            float radius, int damage)
        {
            ...
            for (int i = 0; i < size; i++)
                if (results[i].TryGetComponent(out IGameEntity target) && target.HasTag(GameEntityAPI.CharacterTag))
                    if(gameContext.TakeDamage(target, damage, instigator))
                        break;
        }
    }
}